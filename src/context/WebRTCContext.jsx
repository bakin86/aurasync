...
export function WebRTCProvider({ children }) {
  ...
  const joinCall = useCallback(async (cId, withVideo = false) => {
    try {
      ...
      setChannelId(cId);
      setInCall(true);
      socket.emit("call_join", { channelId: cId, withVideo });
    } catch (err) {
  ...
  const leaveCall = useCallback(() => {
    ...
    peersRef.current = {};
    iceBufRef.current = {};
    socket?.emit("call_leave", { channelId: channelIdRef.current });
  ...
  // Socket event listeners — always active regardless of page
  useEffect(() => {
    if (!socket) return;
  ...
  }, [socket, buildPeer, upsert]);

  return (
    <WebRTCContext.Provider value={{
      inCall, isMuted, isCameraOff, isScreenSharing,
      localStream, screenStream, participants, channelId,
